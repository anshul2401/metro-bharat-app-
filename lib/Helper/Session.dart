// ignore_for_file: avoid_print, unnecessary_brace_in_string_interps, file_names
...
Future<void> saveUserDetail(String id, String name, String email, String mobile,
    String profile, String type, String status) async {
...
String? convertToAgo(DateTime input, int from) {
  ...
  if (diff.inDays >= 1) {
    if (from == 0) {
      var newFormat = DateFormat.yMMMMd('hi').add_jm();
      final newsDate1 = newFormat.format(input).toUpperCase();
      return newsDate1 + ' IST';
    } else if (from == 1) {
    ...
    if (input.minute == 00) {
      ...
      if (from == 2) {
        return ' ${diff.inHours} घंटे ${input.minute} मिनट पहले';
      } else {
        return '${diff.inHours} घंटे ${input.minute} मिनट पहले';
      }
    }
    ...
    return '${diff.inSeconds} सेकंड पहले';
  } else {
    return 'अभी';
  }
}
